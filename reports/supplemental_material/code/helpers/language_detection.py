...
# building a set of English words for vocabulary check
...
def get_german_confidence(lyrics: str) -> float:
    """Returns lingua's confidence score for German."""
    results = DETECTOR.compute_language_confidence_values(lyrics)
    for r in results:
        if r.language == Language.GERMAN:
            return r.value
    return 0.0
