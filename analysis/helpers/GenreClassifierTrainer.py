...
class GenreClassifierTrainer:
    ...
    def fit_with_bayesian_optimization(
        ...
        if (
            ...
            print(
                "Calculating permutation importance on internal validation split (to avoid test leakage)..."
            )

            # Create an internal train/val split solely for unbiased feature importance
            ...
            # Train model wrapper specifically for VIP on the subset
            vip_pipeline = self._create_pipeline(
                self.best_params_, n_estimators=n_estimators_final
            )
            vip_pipeline.fit(X_tr_sub, y_tr_sub)
            ...
            self.vip_ = permutation_importance(
                ...
                y_val_sub,
                random_state=self.random_state,
                n_repeats=5,  # Standard default, can be adjusted
                n_jobs=-1,
            )
