...
class STMTopicModeler:
    ...
    Parameters
    ----------
    use_genre_prevalence : bool, default=True
        Whether to include genre as a prior prevalence covariate in the STM model.
    ...
    def search_k_with_heldout(
        ...
        k_grid : list of k to tune in int, default=(2, 20)
        ...
        self.documents, self.meta = self._prepare_stm_data(X_artist, artist_genres)

        print(f"Tuning number of topics (K range: {self.k_grid})...")
        self.search_results_ = self._run_searchK(
            self.documents, self.meta, X_artist=X_artist, vocab=vocab
        )
    ...
    def _save_tuning_history_plot(self):
        """Plot tuning history of held-out likelihood vs K and highlight local maxima."""
        if not hasattr(self, "search_results_"):
            raise ValueError(
                "No search results found. Call search_k_with_heldout() first."
            )
        if self.model_dir:
            plot_path = self.model_dir / "stm_search_k_plot.png"
        else:
            pass
        ...
        print(f"Tuning history plot saved to {plot_path}")

    def fit(self, k, X_artist=None, artist_genres=None, vocab=None):
        """Fit STM model with selected K on full training data."""
        if X_artist is not None and artist_genres is not None and vocab is not None:
            self.vocab_ = list(vocab)
            print("Preparing data for final model fit...")
            self.documents, self.meta = self._prepare_stm_data(X_artist, artist_genres)
        elif not hasattr(self, "documents") or not hasattr(self, "meta"):
            raise ValueError(
                "Must provide X_artist, artist_genres, and vocab to fit(), "
                "or call tune_and_fit() first to prepare data."
            )
        self.K_ = k
        print(f"Fitting STM model with K = {self.K_}...")
        self.stm_model_ = self._fit_stm(self.documents, self.meta, self.K_)
...
"""
    ...
    def _save_model(self):
        ...
        tuning_path = self.model_dir / "stm_k_search.csv"

        if hasattr(self, "stm_model_"):
            self.base.saveRDS(self.stm_model_, str(model_path))

        if hasattr(self, "search_results_"):
            tuning = pd.DataFrame(
                {
            ...
            self._save_tuning_history_plot()
