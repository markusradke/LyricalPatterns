...
y_pred <- read_csv(
  "models/classificator_selected_rf_tuned_dc/y_test_preditions.csv"
)
y_test <- read_csv("data/X_test_metadata_dc.csv")$dc_detailed |> as.factor()
conf_matrix <- table(y_test, y_pred$prediction)
...
ggplot(conf_matrix_labelled, aes(factor(Var2), fct_rev(y_test), fill = Freq)) +
  geom_tile() +
